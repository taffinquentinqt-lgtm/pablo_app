...
window.generateShareCard = function() {
    ...
    const r = 40;
    ctx.beginPath();
    ctx.roundRect(16, 16, W-32, H-32, r);
    ctx.stroke();
    ...
    const drawContent = () => {
        ...
        if (photoSrc) {
            ...
            img.onerror = () => { drawPlaceholder(ctx, cx, cy, photoR, name); drawText(); document.getElementById('share-canvas-wrap').classList.add('open'); };
            img.src = photoSrc;
        } else {
            drawPlaceholder(ctx, cx, cy, photoR, name);
            ctx.restore();
            drawText();
            document.getElementById('share-canvas-wrap').classList.add('open');
        }
        if (photoSrc) { ctx.restore(); return; }
        ctx.restore();
    };

    const drawPlaceholder = (ctx, cx, cy, r, name) => {
        const g2 = ctx.createLinearGradient(cx-r, cy-r, cx+r, cy+r);
        g2.addColorStop(0, '#1c1710');
        g2.addColorStop(1, '#2a2215');
        ctx.fillStyle = g2;
        ctx.fill();
    ...
    const drawText = () => {
        ...
        // Race
        if (breed) {
            ctx.font      = '400 38px Outfit, Arial, sans-serif';
            ctx.fillStyle = '#c8922a';
            ctx.fillText(breed, W/2, 740);
        }

        // Poids
        if (weight) {
            ctx.font      = '300 30px Outfit, Arial, sans-serif';
            ctx.fillStyle = 'rgba(240,232,216,0.4)';
            ctx.fillText(weight, W/2, 790);
        }
        ...
        ctx.lineWidth   = 1;
        ctx.beginPath();
        ctx.moveTo(160, 920); ctx.lineTo(W-160, 920);
        ctx.stroke();
